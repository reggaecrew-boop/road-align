...
<head>
  ...
  <script>
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker.register("./sw.js?v=18_8_11");
    }
  </script>
...
<div class="wrap">
  <div class="header" id="topHeader">
    <div class="title">
      道路線形計算（iPadオフラインPWA） <span class="badge" id="appVerBadge">v18.8.11</span>
...
const computeCrossSide = (scope, side, staKey)=>{
  ...
  for (const r of segs) {
    ...
    rows.push({ id: r.id, start, end, mode: (r&&r.mode)||'pct', slopePct, ratioX: r?.ratioX, ratioDir: r?.ratioDir, stepDz, span, dzSlope, zEnd: z });
    start = end;
  }
...
const computeCrossRowsFromSegs = (segs)=>{
  ...
  for (const r of segs) {
    ...
    z += dzSlope;
    if (Number.isFinite(stepDz) && Math.abs(stepDz) > 0) z += stepDz;
    rows.push({ start, end, slopePct, stepDz, dzSlope, zEnd: z });
    start = end;
  }
  return { rows, lastEnd: start, lastZ: z, warnings };
};
  ...
  const drawPreview = (tok)=>{
    ...
    if (tblEl) {
      ...
      const rowsToHtml = (calc, sideName)=>{
        const lines = calc.rows.map(r=>`${r.start.toFixed(3)}→${r.end.toFixed(3)} : ${r.slopePct.toFixed(3)}%${(r.stepDz&&Math.abs(r.stepDz)>0)?(' / step '+(r.stepDz>0?'+':'')+r.stepDz.toFixed(3)+'m'):''} (Z=${r.zEnd.toFixed(3)})`);
    ...
    <div class="card">
      ...
      <div class="grid grid-3">
        ...
        <div>
          ...
          <div style="height:10px;"></div>
          <button class="btn btn-ok" id="btnQueryElev">標高を計算</button>
        </div>
      </div>

      <div id="qOut" class="mini" style="margin-top:10px;"></div>
      <div class="mini" style="margin-top:8px;">
        ※横断は <b>例外測点</b> があれば優先。例外が前後にあれば <b>ΔZ（中心→指定幅）を線形補間</b> して擦り付け扱いにします。<br/>
        ※縦断がOFFのときは中心標高（GL）が計算できません。
      </div>
    </div>
  `;
